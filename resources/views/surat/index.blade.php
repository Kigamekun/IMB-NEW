    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#tahun').change(function () {
                ...
                if (selectedYear) {
                    // Panggil endpoint untuk mendapatkan data surat
                    $.ajax({
                        url: '{{ route("surat.filter") }}', // Endpoint baru untuk filter data
                        method: 'GET',
                        data: { tahun: selectedYear },
                        success: function (data) {
                            if (data.length > 0) {
                                // Urutkan data langsung di backend, jadi frontend tidak perlu sorting lagi
                                data.forEach(function (item) {
                                    $('#nomorSK-Pemohon').append(
                                        '<option value="' + item.id + '">' + item.nomorSurat + ' an. ' + item.nama + '</option>'
                                    );
                                });
                            } else {
            ...
            // Event listener untuk modal copy data
